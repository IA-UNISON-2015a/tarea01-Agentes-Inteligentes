...
class AgenteReactivoModeloDosCuartos(entornos_o.Agente):
    ...
    def programa(self, percepción):
        ubicacion = percepción

        # Actualiza el modelo interno
        self.modelo = ubicacion
        
        if self.memoria.count("limpiar") > 2:
            return "nada"
        elif not self.memoria or "ir_A" or "ir_B":
            self.memoria.append("limpiar")
            return "limpiar"
        elif self.memoria[-1] is "limpiar":
            if self.modelo is "A":
                self.memoria.append("ir_B")
                return("ir_B")
            else:
                self.memoria.append("ir_A")
                return("ir_A")
